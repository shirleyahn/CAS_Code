...
y_down = -1.0
y_total = y_up-y_down
proposal_mat = lil_matrix(((int(x_total/delta_x)+1)*(int(y_total/delta_y)+1),
                        (int(x_total/delta_x)+1)*(int(y_total/delta_y)+1)))
acceptance_mat = lil_matrix(((int(x_total/delta_x)+1)*(int(y_total/delta_y)+1),
                        (int(x_total/delta_x)+1)*(int(y_total/delta_y)+1)))
mod_num = int(y_total/delta_y)+1

t0 = time()
for i in range(proposal_mat.shape[0]):
    for j in range(proposal_mat.shape[1]):
        if (i/mod_num == j/mod_num and abs(j % mod_num-i % mod_num) == 1) \
                or (abs(j/mod_num-i/mod_num) == 1 and i % mod_num == j % mod_num):
            proposal_mat[i, j] = 0.25

for i in range(acceptance_mat.shape[0]):
    for j in range(acceptance_mat.shape[1]):
        if (i/mod_num == j/mod_num and abs(j % mod_num-i % mod_num) == 1) \
                or (abs(j/mod_num-i/mod_num) == 1 and i % mod_num == j % mod_num):
            ...
            acceptance_mat[i, j] = np.minimum(1.0, new_energy/old_energy)

trans_mat = proposal_mat.multiply(acceptance_mat)
